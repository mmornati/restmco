...
class RestMCO < Sinatra::Application
    ...
    post '/scheduler/status/:jobid/' do
        content_type :json
        RestMCOConfig.restmco_log.debug "Calling /schedstatus url"
        ...
        RestMCOConfig.restmco_log.info "Command schedstatus #{params[:jobid]} executed on filters #{JSON.dump(data[:filters])}"
    ...
    post '/scheduler/output/:jobid/' do   
        content_type :json
        RestMCOConfig.restmco_log.debug "Calling /schedoutput url"
        ...
        Utils.set_filters(sched, data)
        json_response = JSON.dump(sched.query(jobreq).map{|r| r.results})
        RestMCOConfig.restmco_log.info "Command scheoutput #{params[:jobid]} executed on filters: #{JSON.dump(data[:filters])}"
        RestMCOConfig.restmco_log.debug "Response received: #{json_response}"
        json_response
    end
    ...
    post '/scheduler/mco/:agent/:action/' do
        ...
        RestMCOConfig.restmco_log.info "Executing with backend scheduler"
